...
async function buildPaymentWebhookPayload(input: {
  ...
  const settlement = await PayoutModel.findOne({ sourcePaymentId: payment._id })
    ...
    .select({
      ...
      creditTxHash: 1,
      vaultBatchId: 1,
      vaultDepositTxHash: 1,
      vaultReleaseTxHash: 1,
      vaultHeldAt: 1,
      submittedAt: 1,
      bridgeAttestedAt: 1,
      scheduledFor: 1,
    ...
    payload: {
      ...
      data: {
        ...
        payment: {
          ...
          settlementAccountId: payment.settlementAccountId?.toString() ?? null,
          amount: payment.amount,
          currency: payment.currency,
          description: payment.description,
          recurring: payment.recurring?.enabled ?? false,
        ...
        settlement: settlement
          ? {
              ...
              vaultBatchId: settlement.vaultBatchId ?? null,
              vaultDepositTxHash: settlement.vaultDepositTxHash ?? null,
              vaultReleaseTxHash: settlement.vaultReleaseTxHash ?? null,
              vaultHeldAt: settlement.vaultHeldAt ?? null,
        ...
        settlement:
          input.eventType === "settlement.settled"
            ? {
                ...
                netUsdc: 73.5,
                destinationWallet:
                  "GA5ZSEJYB37SP6QEKWECOUYHQP5O6VWQWDXOZ5RX7OHGC3NL4K3XGS5X",
